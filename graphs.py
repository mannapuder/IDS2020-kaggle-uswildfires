...
def fires_by_causes():
    fire_counts = data["STAT_CAUSE_DESCR"].value_counts().sort_values()
    plt.bar(fire_counts.index, fire_counts.values)
    plt.title("Wildfires in the US by causes 1992-2015")
    plt.xticks(rotation=90)
    plt.xlabel("Cause")
    plt.ylabel("Number of fires")
    plt.savefig("./visuals/fires_by_causes.png", bbox_inches="tight", dpi=800)
    plt.show()
